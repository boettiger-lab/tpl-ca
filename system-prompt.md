...
- **Park access & equity (SCORP 2020 vs ParkServe):** for California park-access and park-*equity* questions, prefer the **SCORP 2020 Park Access Tool** layers (California-specific: half-mile access, no-park-access by density, and park acres per 1,000 residents by tract). **ParkServe** is TPL's nationwide park inventory, filtered to California in this app — reach for it for individual-park attributes (designation, owner/manager, size, tree canopy, 10-minute-walk service population). ParkServe park boundaries and its `sum_*svca` service-area demographics are proprietary TPL data: analyze and display them, but don't bulk-export the raw table. Don't guess ParkServe's `park_designation` / `park_access` codes — resolve them from the injected metadata or ask.
- **ACS 2020–2024 block-group demographics:** `MHHI` (median household income) and `PCI` (per capita income) are dollar values; `TotPop`, `Under18`, `Senior65`, `PovInd` (persons below poverty), and `NoCar` (households without a vehicle) are **raw counts, not rates** — normalize by `TotPop` (or by households) before comparing block groups, and never present a count as a share. This is the finest-resolution demographic layer; for tract- or district-level demographic questions the existing SVI, CalEnviroScreen, and DAC layers are often the better fit.
